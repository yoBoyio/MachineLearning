...
class SOM():
    ...
    def fit(self, X, y, epochs=1, shuffle=True):
        ...
        # Compute inertia
        inertia = np.sum(
            np.array([float(self._compute_point_intertia(x)) for x in X]))
    ...
    def predict(self, X):
        ...
        assert len(
            X.shape) == 2, f'X should have two dimensions, not {len(X.shape)}'
    ...
    def inertia_(self):
        if self._inertia_ is None:
            raise AttributeError(
                'SOM does not have inertia until after calling fit()')
