...
def plot_results_3d(p, X_test, y_test, predictions):
    ...
    ax.scatter(X_test[:, 0], X_test[:, 1], X_test[:, 2],
               marker='o', c=y_test, label='targets')
    ax.scatter(X_test[:, 0], X_test[:, 1], X_test[:, 2], marker='.', c=predictions,
               label='predictions')  # kokkines teleies = lathos, kitrines = swsti ektimisi
...
def iris(plotting_results, live_plotting):
    ...
    if(plotting_results):
        ...
        for i in range(3):
            ...
            ax[i].scatter(range(len(y_test[i])),
                          y_test[i], marker='o', color='b')
...
def bitmap(plotting_results, live_plotting):
    ...
    for set in sets:
        ...
        l = LeastSquares()
        ltargets_train, ltargets_test = l.transmute_targets(
            y_train, y_test)
        weights = l.get_weights(X_train, y_train)
        guesses = l.test(weights, X_test, ltargets_test)
        # predictions: exodos, y_test: stoxos
...
def main():
    ...
    if (file == "iris"):
        ...
        if(plotting_results):
            ...
            plt.scatter(range(len(guesses)), guesses, marker='.',
                        color='r')  # kokkinoi kykloi: exwdos (predictions)
            plt.xlabel("protypo")
            plt.ylabel("exodos (r) / stoxos (b)")
        if(plotting_results_3d):
            plot_results_3d(l, X_test, y_test, guesses)
        print("Perceptron classification accuracy",
              accuracy(y_test, guesses), "%")
    plt.show()
